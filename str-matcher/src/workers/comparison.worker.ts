...
self.onmessage = function(e: MessageEvent<WorkerMessage>) {
  ...
  try {
    console.log('Worker использует режим расчета:', calculationMode);

    // Минимальное требуемое количество маркеров
    const minRequired = {
    ...
    const validProfiles = database.filter(profile => {
      ...
      const validMarkers = markersToCompare.filter((marker: string) => {
    ...
    // Отправляем прогресс и результаты
