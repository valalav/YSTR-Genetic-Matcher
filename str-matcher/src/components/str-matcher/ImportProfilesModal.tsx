...
const ImportProfilesModal: React.FC<ImportProfilesModalProps> = ({ isOpen, onClose, onImport }) => {
  ...
  const handleFileUpload = useCallback((event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (e) => {
      ...
      try {
        ...
        setError(`Ошибка парсинга файла: ${err instanceof Error ? err.message : 'Unknown error'}`);
        setPreview([]);
      }
    };
  ...
  const handleImport = useCallback(() => {
    if (preview.length === 0) {
      setError('Нет профилей для импорта');
      return;
    }
      ...
      <div className="bg-white rounded-lg shadow-xl max-w-4xl w-full max-h-[90vh] overflow-hidden">
        ...
        <div className="bg-gradient-to-r from-blue-600 to-indigo-600 text-white px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-xl font-bold">Импорт профилей</h2>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-6 overflow-y-auto max-h-[calc(90vh-120px)]">
          ...
          {preview.length > 0 && (
            <div className="mb-6">
              <h3 className="text-lg font-semibold mb-3">
                Предпросмотр ({preview.length} профилей)
              </h3>
              <div className="border border-gray-200 rounded-lg overflow-hidden max-h-64 overflow-y-auto">
                ...
                {preview.length > 10 && (
                  <div className="p-2 bg-gray-50 text-center text-xs text-gray-500">
                    ... и еще {preview.length - 10} профилей
                  </div>
                )}
              </div>
            </div>
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
            ...
            <ul className="text-sm text-blue-800 space-y-1">
              <li>• Импортированные профили сохраняются только в текущей сессии</li>
              <li>• Они НЕ добавляются в базу данных</li>
              <li>• При совпадении Kit Number приоритет у импортированных профилей</li>
              <li>• После перезагрузки страницы импортированные профили исчезнут</li>
            </ul>
          </div>
        </div>
        ...
        <div className="bg-gray-50 px-6 py-4 flex items-center justify-between border-t">
          <div className="text-sm text-gray-600">
            {preview.length > 0 && `Готово к импорту: ${preview.length} профилей`}
          </div>
