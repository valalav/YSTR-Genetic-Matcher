...
router.post('/upload',
  ...
  asyncHandler(async (req, res) => {
    ...
    const profiles = parseResult.data.map(row => {
      ...
      const profile = {
        kitNumber: row.kitNumber || row.kit_number || row.KitNumber || row['Kit Number'],
      ...
      Object.keys(row).forEach(key => {
        ...
        if (!['kitnumber', 'kit_number', 'kit number', 'name', 'country', 'haplogroup', 'paternal ancestor name'].includes(lowerKey)) {
          if (row[key] && row[key] !== '') {
            markers[key] = row[key];
          }
        }
