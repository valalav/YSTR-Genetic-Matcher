class HaploTree {
        ...
        const getHaplogroupPath = (nodeId) => {
            ...
            while (current) {
                path.push({
                    ...
                    name: current.name,
                    variants: current.variants?.map(v => v.variant || v.snp).filter(Boolean) || [],
                    displayName: current.name // Используем только имя без добавления SNP
                });
    ...
    getBaseHaplogroup(haplogroup) {
        ...
        // Получаем первую букву или букву с цифрой (например, R1)
    ...
    isSubclade(haplogroup, parentHaplogroup) {
        ...
        if (!baseHaplo || !baseParent || baseHaplo !== baseParent) {
            console.log('Base haplogroups mismatch:', { baseHaplo, baseParent });
            return false;
        }
