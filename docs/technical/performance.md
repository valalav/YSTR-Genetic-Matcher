...
### Целевые показатели:
- **Загрузка данных**: < 3 секунд для 50К записей
- **Расчет дистанций**: < 5 секунд для 1000 сравнений
- **API ответы**: < 200мс для check-subclade
- **Фильтрация**: < 1 секунды для 10К записей
- **Использование памяти**: < 500MB для полного набора данных

### Текущие узкие места:
1. **Загрузка больших JSON файлов** - основной bottleneck
2. **Расчеты генетических дистанций** - CPU intensive операции
3. **Проверка субкладов** - множественные запросы к API
4. **Рендеринг больших таблиц** - DOM операции
